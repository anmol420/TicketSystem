...
const analyzeTicket = async (ticket: Ticket) => {
    ...
    const match = response.raw?.match(/```json\s*([\s\S]*?)\s*```/i);
    try {
        const jsonString = match ? match[1].trim() : response.raw?.trim();
        if (!jsonString) throw new Error("No JSON string to parse from AI response.");
